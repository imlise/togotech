...
tbody.innerHTML = await buildRows(drafts);
  ...
  tbody.addEventListener('click', e => {
    ...
    TTComponents.confirm({ title: 'Supprimer le brouillon', message: 'Cette action est irréversible.', danger: true }).then(async ok => {
      if (!ok) return;
      // TT.saveDrafts(TT.getDrafts().filter(d => d.id !== btn.dataset.id));
        await AA.deleteFacture(btn.dataset.id);
