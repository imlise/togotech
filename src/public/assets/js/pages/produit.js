...
async function renderProduits() {
  ...
  try {
    ...
    grid.innerHTML = produits.map(p => {
      const prixFinal = p.prixUnitaire - (p.prixUnitaire * (p.reduction || 0) / 100);
      console.log(p.image);
      ...
      return `
        <div class="card client-card">
          ...
          <div class="client-card__name">${p.nom}</div>
          ...
          <div class="client-card__stats">
            <div>
              <div class="client-card__stat-value text-mono">
                ${p.prixUnitaire.toFixed(2)} FCFA
              </div>
              <div class="client-card__stat-label">Prix initial</div>
            </div>

            <div>
              <div class="client-card__stat-value text-mono">
                ${prixFinal.toFixed(2)} FCFA
              </div>
              <div class="client-card__stat-label">Après réduction (${p.reduction || 0}%)</div>
            </div>
          </div>

          <button class="btn btn--primary btn--sm add-to-cart" data-id="${p.id}" style="width:100%;margin-top:var(--space-4)">
            Ajouter
          </button>
        </div>
      `;
    ...
    grid.querySelectorAll('.add-to-cart').forEach(btn => {
      btn.addEventListener('click', () => {
        console.log('Produit ajouté:', btn.dataset.id);
      });
    });
