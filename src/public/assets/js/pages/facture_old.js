...
// initialise l’éditeur à partir de l’URL,
// charge les réglages et la liste des clients,
// branche les événements,
// prépare le formulaire et l’aperçu.
function init() {

  // 🔹 Récupère les paramètres dans l'URL
  // ex: facture.html?id=123&type=proforma
  const params = new URLSearchParams(location.search);

  // 🔹 Récupère l'id du document (si on est en mode édition)
  editId = params.get('id');

  // 🔹 Détermine le type de document
  // priorité :
  // 1. ?type=...
  // 2. si ?edit existe → null
  // 3. sinon "facture" par défaut
  docType = params.get('type') || (params.get('edit') ? null : 'facture') || 'facture';

  // 🔹 Si le type est "proforma", on applique ce mode
  if (params.get('type') === 'proforma') setType('proforma');

  // 🔹 Charge les paramètres globaux (TVA, conditions, etc.)
  loadSettings();

  // 🔹 Remplit la liste des clients (autocomplete)
  populateClients();

  // 🔹 Attache tous les événements (click, input, etc.)
  bindEvents();

  // 🔹 Récupère un client passé dans l'URL
  // ex: ?client=Dupont
  const clientParam = params.get('client');

  // 🔴 CAS 1 : on modifie une facture existante
  if (editId) {

    // 🔹 Charge les données du document existant
    loadDocument(editId);

  } else {

    // 🟢 CAS 2 : nouvelle facture
    ...
    if (clientParam) {
      ...
      // 🔹 Remplit le champ client
      $('fClient').value = decodeURIComponent(clientParam);

      // 🔹 Remplit automatiquement email, téléphone, etc.
      autofillClient();
    }
  }
}

// charge les valeurs par défaut comme les conditions et le taux de TVA
function loadSettings() {

  // 🔹 Remplit le champ "conditions" du formulaire
  // avec la valeur stockée dans les paramètres globaux
  $('fConditions').value = settings.conditions;

  // 🔹 Remplit le champ TVA
  // si une TVA est définie dans les settings → on l’utilise
  // sinon → valeur par défaut = 18%
  $('fTva').value = settings.tva || 18;
}

// remplit la liste des clients disponibles.
  ...
  //bascule entre facture et proforma,
  // met à jour l’interface et le libellé du document.
  ...
  // complète automatiquement email, téléphone et adresse à partir du client sélectionné.
...
/////////////////////    Gestion des lignes de documents       ////////////////////////////////////////////////////////////////////////
  ...
  // affiche les lignes dans l’interface et attache les gestionnaires d’événements
  ...
  // met à jour une ligne modifiée par l’utilisateur.
  function updateLine(id, key, val) {
    const line = lines.find(l => l.id === id);
    if (line) { line[key] = val; syncAll(); }
  }
  //gère l’ajout d’une image sur une lign
...
/////////////////////////////////////       Calculs      //////////////////////////////////////////////////////////////////////
  //calcule le montant HT, TVA, TTC et les totaux de chaque ligne
  function calcTotals() {
...
// rafraîchit tous les totaux,
// met à jour l’aperçu PDF,
// synchronise la vue avec l’état courant du formulaire.
  function syncAll() {
  ...
  // assemble un objet complet représentant la facture/proforma en cours de saisie.
...
/////////////////////////    Enregistrement et cycle du document    //////////////////////////////////////////////////////////////////////////////////
  // enregistre le document comme brouillon ou comme document final
  ...
  // crée un nouveau document basé sur un document existant.
  ...
  //remplit le formulaire à partir d’un document déjà enregistré ou d’un brouillon.
